...
// *********************************************************************
{
  ...
  char* gainHistoName[30], gainHistoFileName[50],
  ...
  for ( int volume_i = 0; volume_i < 2; volume_i++ ) {
    ...
    for ( int particle_i = 0; particle_i < 5; particle_i++ ) {
      ...
      sprintf(syscmd, "mkdir -p gainHistos/%c%s/", particleName[particle_i], volumeName[volume_i]);
      system((const char*)syscmd);
      ...
      for ( int energy_i = 0; energy_i < 7; energy_i++ ) {
        ...
        // Define Tree names
        sprintf(gainHistoName, "%cGainHisto%s", particleName[particle_i], volumeName[volume_i]);
        ...
        sprintf(pngName, "gainHistos/%c%s/%dMeV.png", particleName[particle_i], volumeName[volume_i], runEnergy[energy_i]);
