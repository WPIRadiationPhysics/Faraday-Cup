...
for particle=['e' 'p' 'o' 'n' 'g']
  for runID=0:6
    ...
    % Define cascade file
    cascadeDataFile = strcat('cascadeHistos/', particle, 'Cascade-', int2str(runID), '.dat');
    ...
    try % ignores empty files
      ...
      E_r = cascadeData(:, 3)/energies(runID+1); E_z = cascadeData(:, 4)/energies(runID+1);

      % Display momenta on the grid with quiver
      quiver(z, r, E_z, E_r);

      % Format figure
      titleString = strcat(particle, ' Directed Energy Deposition at:   ', num2str(energies(runID+1)), ' MeV');
      title(titleString);
      ...
      fileName = strcat('cascadeHistos/', particle, 'Cascade-', int2str(runID), 'MeV.png');
      saveas(gcf, fileName)
